...
export default function Register() {
    ...
    async function onSubmit(values) {
        ...
        const photoName = `${Date.now()}-${values.photo.name}`;

        // Prepare the photo data to be sent
        const formData = new FormData();
        formData.append('photo', values.photo, photoName);

        // // Save the photo in the public/uploads directory
        // await fetch('http://localhost:3000/api/auth/upload-photo', {
        //     method: 'POST',
        //     body: formData,
        // });

        // Include the photo name in the form data to be sent in the main POST request
        const dataToSend = {
            ...values,
            photo: photoName,
        };
